...
module.exports = function(params, cb) {
	...
	function fixStorylineMobileJs(cb) {
		...
		try {
			...
			// This somehow fixes landscape mode on iPad in Storyline.
			playerCompiledJsString = playerCompiledJsString.replace(/var\s+k=\s*j\s*-\s*window\.innerHeight/g, 'var k=0');
			playerCompiledJsString = playerCompiledJsString.replace(/if\(self!=top\)a=a\.replace\("framewrap","portrait\s\.framewrap"\)\.replace\("interstitial","portrait\s\.interstitial"\),player\.rotatePortraitFramedStyleSheet=document\.createElement\("style"\),player\.rotatePortraitFramedStyleSheet\.innerHTML=a,document\.body\.appendChild\(player\.rotatePortraitFramedStyleSheet\)/g, ';');
