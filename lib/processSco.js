...
module.exports = (params, mainCb) => {
  ...
  function fixArticulateRiseJs(cb) {
    ...
    try {
      ...
      fs.writeFileSync(playerCompiledJsPath, playerCompiledJsString, { encoding: 'utf8' });
      cb(null, 'articulate_rise');
    } catch (err) {
      return cb(err);
    }
  }

  function fixCamtasia(cb) {
    const playerCompiledJsPath = path.join(path.dirname(p.pathOfManifest), 'scripts', 'config_xml.js');
    if (!fs.existsSync(playerCompiledJsPath)) return cb();

    try {
      var playerCompiledJsString = fs.readFileSync(playerCompiledJsPath, { encoding: 'utf8' });
      var camtasiaType = 'camtasia';
      // Check the config file for quizzes
      if (playerCompiledJsString.match(/trackType=\"Quiz\"/g)) {
        camtasiaType = 'camtasia_quiz';
      }
      cb(null, camtasiaType);
    } catch (err) {
      return cb(err);
    }
  }
