...
module.exports = (params, mainCb) => {
  ...
  function fixStorylineMobileJs(cb) {
    ...
    try {
      let playerCompiledJsString = fs.readFileSync(playerCompiledJsPath, { encoding: 'utf8' });
  ...
  function fixStorylineWindowMode(cb) {
    ...
    try {
      let storyHtmlString = fs.readFileSync(storyHtmlPath, { encoding: 'utf8' });
  ...
  function fixCaptivateFlashPlayerScaling(cb) {
    try {
      fstools.walkSync(path.dirname(p.pathOfManifest), '.html|.htm$', (_path) => {
        let fileHtmlString = fs.readFileSync(_path, { encoding: 'utf8' });
  ...
  function fixCaptivateWindowMode(cb) {
    ...
    try {
      let scormUtilitiesJsString = fs.readFileSync(scormUtilitiesJsPath, { encoding: 'utf8' });
